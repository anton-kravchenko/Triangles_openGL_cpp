...
float 	Triangle::EGDE_LENGTH 	= -0.99;//very bad
...
Point* Triangle::rotate_point(Point* p, float angle)
{
...
Line* Triangle::rotate_vector(Line* vector, float angle)
{
	float vx = vector->b->x;
	float vy = vector->b->y;
...
void Triangle::draw_doodle()
{
	...
	for(int a = 0; a < 150; a++, rotate_angle-= 0.005)
	{
		for(int i = 0; i < 3; i++)
		{
	...
	//[1]
			...
			rotate_vector(vector, rotate_angle);
